...
{
    if ((n >= angka) && (data[angka - 1].status == true))
    {
        data[angka - 1].status = false;
        printf("Data telah dihapus\n");
    }
    else if ((n >= angka) && (data[angka - 1].status == false))
    {
        printf("Data sudah kosong\n");
    }
    else if (n < angka)
    {
        printf("Data tidak ditemukann\n");
    }
    printf("\n");
}
